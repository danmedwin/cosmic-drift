...
export default function CosmicWorkshop() {
  ...
  var _goOv = useState(function() { try { return sessionStorage.getItem("cd_transition_from") === "game"; } catch(e) { return false; } }), showGameOverlay = _goOv[0], setShowGameOverlay = _goOv[1];
  ...
  useEffect(function() {
    try {
      ...
      if (from === "game") {
        ...
        setTimeout(function() { setSplashEntering(false); }, 900);
      }
    } catch(e) {}
  }, []);
  ...
  return React.createElement("div", { style: { position: "fixed", inset: 0, background: "#0b0c1a", fontFamily: "'Quicksand',sans-serif", overflow: "hidden" } },
    ...
    showGameOverlay && React.createElement("div", {
      style: { position: "fixed", inset: 0, zIndex: 100, overflow: "hidden" }
    }, React.createElement("div", {
      style: { position: "absolute", inset: 0, background: "#0b0c1a", animation: "cdGameOverlayExit 0.68s ease-in both" },
      onAnimationEnd: function() { setShowGameOverlay(false); }
    })),

    // ═══ SPLASH SCREEN ═══
    screen === "splash" && React.createElement("div", { onTouchStart: handleSplashTouchStart, onTouchEnd: handleSplashTouchEnd, style: { position: "relative", zIndex: 1, display: "flex", flexDirection: "column", height: "100%", background: "radial-gradient(ellipse 90% 60% at 50% 28%, #0e0820 0%, #0a0618 45%, #06040f 100%)", animation: splashEntering ? "cdSplashEnterFromGame 0.4s ease-out 0.5s both" : (splashTransition === "exit" ? "cdSplashExitShrink 0.62s ease-in both" : "none"), willChange: "transform, filter" } },
